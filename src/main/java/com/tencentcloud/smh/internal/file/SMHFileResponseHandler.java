...
public class SMHFileResponseHandler extends AbstractSmhResponseHandler<SMHFile> {
    ...
    @Override
    public SmhServiceResponse<SMHFile> handle(SmhHttpResponse response) throws Exception {
        SMHFile object = new SMHFile();
        SmhServiceResponse<SMHFile> smhResponse =  new SmhServiceResponse<SMHFile>();
        object.setObjectContent(
                new SMHFileInputStream(response.getContent(), response.getHttpRequest()));
        object.setRequestId(response.getHeaders().get(Headers.REQUEST_ID));
        smhResponse.setResult(object);
        return smhResponse;
    }
